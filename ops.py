...
def batchnorm(x, momentum=0.9, epsilon=1e-5, train=True, name='bn'):
...
def conv2d_block(input, out_dim, k, s, is_train, name):
    with tf.name_scope(name), tf.variable_scope(name):
        ...
        net = batchnorm(net, train=is_train, name='bn')
        net = relu(net)
        return net
...
def pwise_block(input, output_dim, is_train, name, bias=False):
    with tf.name_scope(name), tf.variable_scope(name):
        ...
        out=batchnorm(out, train=is_train, name='bn')
        out=relu(out)
        return out
...
def res_block(input, expansion_ratio, output_dim, stride, is_train, name, bias=False, shortcut=True):
    with tf.name_scope(name), tf.variable_scope(name):
        ...
        net = batchnorm(net, train=is_train, name='pw_bn')
        net = relu(net)
        # dw
        net = dwise_conv(net, strides=[1, stride, stride, 1], name='dw', bias=bias)
        net = batchnorm(net, train=is_train, name='dw_bn')
        net = relu(net)
        # pw & linear
        net = conv_1x1(net, output_dim, name='pw_linear', bias=bias)
        net = batchnorm(net, train=is_train, name='pw_linear_bn')
